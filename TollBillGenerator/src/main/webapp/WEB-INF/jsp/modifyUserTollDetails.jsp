...
<script type="text/javascript">
	...
	function populateMyData()
	{
		...
		<c:forEach items="${locationsList}" var="l">
			if("${l.tapproval}" == "APPROVED" || "${l.tapproval}" == "approved"){
				dat = [];
	...
	function populateForm() {
		...
		document.getElementById("ftid").value = ${modifyId};
	...
	{
		var fsource = document.getElementById("source");
		temp = '${defaultSource}';
		setDefaults(fsource,temp);
		applyDropDown();
		
		
		var fdestination = document.getElementById("destination");
		temp = '${defaultDestination}';
		setDefaults(fdestination,temp);
		applyVtypeDropDown();
		
		var fvtype = document.getElementById("vtype");
		temp = '${defaultVtype}';
		setDefaults(fvtype,temp);
		applyPriceDropDown();
		
		var foneortwo = document.getElementById("oneortwo");
		temp = '${defaultOneorTwo}';
		setDefaults(foneortwo,temp);
		applyPriceDropDown();
		
		var fprice = document.getElementById("price");
		temp = '${defaultPrice}';
		document.getElementById("foldprice").value = '${defaultPrice}';
		setDefaults(fprice,temp);
		
		
		var jdate = document.getElementById("jdate");
		jdate.defaultValue = '${defaultJDate}';
	}
		
	function setDefaults(elm,temp)
	{
		for(var i, j = 0; i = elm.options[j]; j++) {
			if(i.value == temp) {
			elm.selectedIndex = j;
			break;
		}
...
<body style="background-color: #f0f1f2;">
		...
		<div class="row">
			<div class="col-md-6 offset-md-3">
				<div class="card">
					...
					<h2 align="center">Modify Planned Journey</h2>
	...
	<form id="myform" action="/confirmModifyPayment" method="post">
		<input type="hidden" name="fsource" id="fsource" value=""> 
		<input type="hidden" name="fdestination" id="fdestination" value="">
		<input type="hidden" name="fvtype" id="fvtype" value=""> 
		<input type="hidden" name="foneortwo" id="foneortwo" value=""> 
		<input type="hidden" name="fprice" id="fprice" value=""> 
		<input type="hidden" name="foldprice" id="foldprice" value=""> 
		<input type="hidden" name="ftid" id="ftid" value="">
		<input type="date" name="fjdate" id="fjdate" value="" hidden="hidden">
	</form>
